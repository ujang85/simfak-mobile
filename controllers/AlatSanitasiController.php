...
use app\models\AlatSanitasiSearch;
 ...
 */
class AlatSanitasiController extends Controller
{
    /**
     * @inheritdoc
     */
    ...
    {    
        $searchModel = new AlatSanitasiSearch();
    ...
    public function actionLap_aset_sanitasi()
    {    
        $searchModel = new AlatSanitasiSearch();
        $dataProvider = $searchModel->search(Yii::$app->request->queryParams);

        return $this->render('lap_aset_sanitasi', [
            'searchModel' => $searchModel,
            'dataProvider' => $dataProvider,
        ]);
    }
            ...
            if($request->isGet){
                return [
                    ...
                    'footer'=> Html::button('Close',['class'=>'btn btn-default pull-left','data-dismiss'=>"modal"]).
                     ...
                     $model->satuan_periodepm = $model->satuan_periodepm;
                     $model->konversi_pm = $model->periode_pm * $model->satuan_periodepm;
                     //$model->periode_pm  . " " . $model->periode->satuan_periode
                    // $konversi=$model->periode_pm * $model->satuan_periodepm;
                   //  $model->konversi_pm ="P".$konversi."D";
                     $model->life_time = $model->life_time;
                     $model->cost_tahun=$model->cost_tahun;
                     $model->satuan_periodekl = $model->satuan_periodekl;
                     $model->konversi_kl = $model->periode_kl * $model->satuan_periodekl;
    ...
    {   
     ...
     //  $AlatRs=TbAlatRs::findOne($inven_rs='AM-0011');
       //   $Alat=TbAlatRs::findAll($invenId);
        $Alat=AlatMedis::find()->where(['inven_rs'=>$invenId])->one();
    ...
    {   
     ...
     //  $AlatRs=TbAlatRs::findOne($inven_rs='AM-0011');
       //   $Alat=TbAlatRs::findAll($invenId);
        $KodeAlat=AlatMedis::find()->where(['kode_alat'=>$kode])->one();
